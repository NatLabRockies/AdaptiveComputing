...
int main (int argc, char* argv[])
{
	...
	// Thermal diffusivity
	// pp.get("thermal_diffusivity",alpha);
    }
    ...
    amrex::MultiFab alpha(ba, dm, Ncomp, Nghost);
    amrex::Array<amrex::MultiFab, AMREX_SPACEDIM> alpha_face;
    for (int i=0; i<AMREX_SPACEDIM; ++i){
	    amrex::BoxArray baf= ba.surroundingNodes(i);
	    alpha_face[i].define(baf,dm,Ncomp,0);
    }
    ...
    for (int step = 1; step <= nsteps; ++step)
    {
        ...
        for ( amrex::MFIter mfi(phi_old); mfi.isValid(); ++mfi )
        {
            ...
            const amrex::Array4<amrex::Real>& alpha_i = alpha.array(mfi);
            const amrex::Array4<amrex::Real>& alpha_face_0 = alpha_face[0].array(mfi);
            const amrex::Array4<amrex::Real>& alpha_face_1 = alpha_face[1].array(mfi);
            const amrex::Array4<amrex::Real>& alpha_face_2 = alpha_face[2].array(mfi);

	    auto bxg=amrex::grow(bx,1);

            amrex::ParallelFor(bxg, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
	        alpha_i(i,j,k) = get_thermal_diffusivity(phiOld(i,j,k));
	    });
            auto bx0=amrex::surroundingNodes(bx,0);
            amrex::ParallelFor(bx0, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                alpha_face_0(i,j,k) = (alpha_i(i,j,k)+alpha_i(i-1,j,k))/2; 
            });
            auto bx1=amrex::surroundingNodes(bx,1);
            amrex::ParallelFor(bx1, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                alpha_face_1(i,j,k) = (alpha_i(i,j,k)+alpha_i(i,j-1,k))/2;
            });	    
            auto bx2=amrex::surroundingNodes(bx,2);
            amrex::ParallelFor(bx2, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                alpha_face_2(i,j,k) = (alpha_i(i,j,k)+alpha_i(i,j,k-1))/2;
            });	    
		    ...
		    ( (alpha_face_0(i+1,j,k)*(phiOld(i+1,j,k) - phiOld(i  ,j,k))
		    -  alpha_face_0(i  ,j,k)*(phiOld(i  ,j,k) - phiOld(i-1,j,k)))/(dx[0]*dx[0])
                    + (alpha_face_1(i,j+1,k)*(phiOld(i,j+1,k) - phiOld(i,j  ,k)) 
                    -  alpha_face_1(i,j  ,k)*(phiOld(i,j  ,k) - phiOld(i,j-1,k)))/(dx[1]*dx[1])
                    + (alpha_face_2(i,j,k+1)*(phiOld(i,j,k+1) - phiOld(i,j,k  )) 
                    -  alpha_face_2(i,j,k  )*(phiOld(i,j,k  ) - phiOld(i,j,k-1)))/(dx[2]*dx[2])
		    );
