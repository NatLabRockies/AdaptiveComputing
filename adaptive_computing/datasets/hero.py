...
class HeroDataset(DatasetBase):
    def __init__(self, params, machine_names, output_field_path, n_fidelity=1, blocking=False,
                 task_formatter=None, nan_behavior='mask_ignore', hero_client=None, queue_name=None):
        """
        ...
        Args:
            ...
            hero_client: Optional Hero client instance.  Pass a LocalHeroClient to use a
                local JSON file instead of the real Hero service (no credentials required).
                When None, the real HeroClient is used (default).
        """
        ...
        self._hero_todo = [np.empty([0,1],dtype=bool)]*self.n_fidelity if False else None  # set below after super().__init__
        self._hero_task_id = None  # set below

        super().__init__(params, n_fidelity=n_fidelity, nan_behavior=nan_behavior)

        # boolean indicating if the sample evaluation is queued to be run by a Hero manager
        self._hero_todo = [np.empty([0,1],dtype=bool)]*self.n_fidelity
        # string generated by hero to identify each simulation run's inputs and outputs
        self._hero_task_id = [np.empty([0,1], dtype='<U72')]*self.n_fidelity

        self.hero_authenticate(machine_names, hero_client=hero_client, queue_name=queue_name)
    ...
    def hero_authenticate(self, machine_names, hero_client=None, queue_name=None):
        """Connect to the Hero backend and find or create per-fidelity queues.

        Args:
            machine_names: List of logical machine names stored in task metadata.
            hero_client:   Optional Hero client instance.  If not provided here,
                           the instance stored from the previous call (or __init__)
                           is reused.  Pass a LocalHeroClient to skip credentials.
        """
        # Prefer the explicitly provided client; fall back to the stored one.
        if hero_client is None:
            hero_client = getattr(self, '_hero_client', None)
        self._hero_client = hero_client

        if hero_client is not None:
            # Local / custom client — no env vars or network auth required.
            hero_queue     = getattr(hero_client, 'queue_name', 'local')
            application_id = getattr(hero_client, 'application_id', 'local')
            hero = hero_client
        else:
            # Real Hero client path.
            set_hero_env_vars()
            try:
                HERO_ENV  = get_env_variable('HERO_ENV', 'dev')
                HERO_PROJECT = get_env_variable('HERO_PROJECT')
                hero_queue   = queue_name or get_env_variable('HERO_QUEUE')
