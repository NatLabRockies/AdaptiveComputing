...
class Model:
    def __init__(self, simulations, params, mod_ops):
        ...
        self.x_data = [np.empty([0,self.n_dim])]*self.n_fl # x_data is a list of length n_fl. Each entry will be an n_samp[i_fl] x n_dim np array
        # the function values at these coordiantes:
        self.y_data = [np.empty([0,1])]*self.n_fl # y_data is a list of length n_fl. Each entry will be an n_samp[i_fl] x 1 np array
        # boolean indicating if the data is non-NaN and within user-specified bounds
        self.unmasked_data = [np.empty([0,1],dtype=bool)]*self.n_fl # unmasked_data is a list of length n_fl. Each entry will be an n_samp[i_fl] x 1 np array
        # boolean indicating if the sample is queued to be run by a Hero worker
        ...
        # set up the GPs Gaussian Process models (AKA the Kriging model)
        from ac_common.surrogate_wrappers import SMTWrapper
        self.surrogate = SMTWrapper(self.n_fl, self.multifidelity, self.mixed_type, xlimits=self.xlimits, xtypes=self.xtypes)
    ...
    def add_xnum_sample(self,fidelity_level,x_eval_num,y_eval=None,viz_ops=None,frame_id=None,train=True):
        from ac_common.static_sampling import add_xnum_sample
        add_xnum_sample(self,fidelity_level,x_eval_num,y_eval,viz_ops,frame_id,train)

    def queue_hero_sample(self,fidelity_level,x_eval_num,y_surrogate=True,viz_ops=None,frame_id=None):
        from ac_common.static_sampling import queue_hero_sample
        queue_hero_sample(self,fidelity_level,x_eval_num,y_surrogate,viz_ops,frame_id)
    
    def sync_hero_results(self,viz_ops=None,train=True):
        from ac_common.static_sampling import sync_hero_results
        sync_hero_results(self,viz_ops,train)

    def wait_for_workers(self,viz_ops=None,train=True):
        from ac_common.static_sampling import wait_for_workers
        wait_for_workers(self,viz_ops,train)
