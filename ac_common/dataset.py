...
class DataSet:
    def __init__(self, simulations, params, ds_ops, n_out=1):
        ...
        # variables for hero and masking:
        ...
        # the sample-space coordinates:
        self.x_data_skipped = [np.empty([0,self.n_in])]*self.n_fl # x_data is a list of length n_fl. Each entry will be an n_samp[i_fl] x n_in np array
        # the function values at these coordiantes:
        self.y_data_skipped = [np.empty([0,self.n_out])]*self.n_fl # y_data is a list of length n_fl. Each entry will be an n_samp[i_fl] x n_out np array
