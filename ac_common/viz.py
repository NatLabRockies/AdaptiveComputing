    ...
    if n_fl == 1:
        ...
        if viz_ops.animation_1d:
            ...
            for i in range(len(x_plot)):
                #y_plot[i] = dataset.funcs[0](x_plot[i])
                y_plot[i] = dataset.eval_xnum(0,x_plot[i])
        ...
        if viz_ops.animation_1d:
            ...
            y_plot = np.zeros_like(x_plot)
            for i in range(len(x_plot)):
                #y_plot[i] = dataset.funcs[-1](x_plot[i])
                y_plot[i] = dataset.eval_xnum(-1,x_plot[i])
            y_gp_plot = surrogate.predict_values(x_plot)
            y_gp_plot_var  =  surrogate.predict_variances(x_plot)
            fig = plt.figure(figsize=[10,10])
            ax = fig.add_subplot(111)
        ...
        raise Exception('Animation for 3+ fidelity levels is not supported but the user can access dataset.dataset.x_data and dataset.dataset.y_data for plotting.')
...
def viz_finalize(dataset,surrogate,viz_ops,frame_id):
    ...
    if n_fl == 1:
        ...
        raise Exception('Plotting for 3+ fidelity levels is not supported but the user can access dataset.dataset.x_data and dataset.dataset.y_data for plotting.')
