...
int main(int argc, char*argv[])
{
    ...
    */
	...
	std::cout << "\nSTARTING MICRO SIMULATION " << x+1 << ".\n" << std::endl;

	float* hf_query = max_x_query;

        std::cout << "Query: " << hf_query[0] << ", " << hf_query[1] << ", " << hf_query[2] << ", " << hf_query[3] << ", " << hf_query[4] << ", " << hf_query[5] << ", " << hf_query[6] << ", " << hf_query[7] << std::endl;
        ...
        std::cout << "Result: " << y_val[0] << ", " << y_val[1] << ", " << y_val[2] << ", " << y_val[3] << ", " << y_val[4] << ", " << y_val[5] << std::endl;
	
	PyObject *add_result = PyObject_CallMethod(myModule, "add_xnum_sample_6d", "OiOOOOOOOO", my_dataset, -1, x_val_arr, y_val_arr, my_surrogate0,my_surrogate1,my_surrogate2,my_surrogate3,my_surrogate4,my_surrogate5);
        if (!add_result || PyErr_Occurred()) {
            std::cerr << "Error calling Python method 'add_xnum_sample_6d'.\n";
            Py_DECREF(myModule);
            Py_DECREF(my_dataset);
            Py_DECREF(my_surrogate0);
            Py_DECREF(my_surrogate1);
            ...
            std::cout << "Added sample to unmasked data.\nTrained 6 surrogates on new unmasked data." << std::endl;
            Py_DECREF(x_val_arr);
            Py_DECREF(y_val_arr);
